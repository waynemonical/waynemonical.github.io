...
plot(first.obj.reg$linear.pca$x[,1])

# this is how genes are effected
# 
plot(first.obj.reg$linear.pca$rotation[1:200,1])
plot(first.obj.reg$linear.pca$rotation[1:200,2])
plot(first.obj.reg$linear.pca$rotation[1:200,3])
plot(first.obj.reg$linear.pca$rotation[1:200,4])

hist(all.obj.lreg$all.boolean$linear.pca$x[,1])


all.obj.lreg$all.hap$linear.pca$x

hist(all.obj.lreg$all.hap$linear.pca$x[,1])
hist(all.obj.lreg$all.hap$linear.pca$x[,2])
hist(all.obj.lreg$all.hap$linear.pca$x[,3])


plot(first.obj.reg$lpca.regression$residuals)
```


```{r}
lapply(all.obj.lreg, FUN = function(x){summary(x$lpca.regression)})
```
